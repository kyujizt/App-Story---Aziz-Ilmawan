...
export default class AddStoryPresenter {
  ...
  async _handleSubmit(description, file) {
    ...
    try {
      const newStory = await addStory({ description, imageFile, location: this.selectedLatLng });

      // 🔔 Kirim push notification dengan deskripsi story yang diunggah
      await sendPushNotification({
        title: "Cerita Baru dari " + (localStorage.getItem("userName") || "Pengguna"),
        body: `Anda telah membuat story baru dengan deskripsi: ${description}`, // ✅ Pastikan deskripsi masuk dalam notifikasi
        location: this.selectedLatLng,
        storyId: newStory?.id, // ✅ Sertakan ID story agar bisa membangun URL notifikasi
        description: description, // ✅ Kirim deskripsi untuk diproses oleh API
      });
      ...
      AddStoryView.showAlert("Gagal menambahkan cerita. Coba lagi.");
    }
  }
